...
void Wishlist::add_to_wishlist(Travel_Package* travel_package) {
    ...
    Travel_Package* new_travel_package = new Travel_Package(*travel_package);
    favorites->add(new_travel_package);
    wishlist_map.insert({ new_travel_package->get_type(), new_travel_package });

}

void Wishlist::delete_from_wishlist(int id) {

    auto it = std::find_if(favorites->begin_ascendent(), favorites->end_ascendent(),
        [id](const Travel_Package* pkg) { return pkg->get_id() == id; });

    if (it == favorites->end_ascendent()) {
        throw ExceptionWishlist("Travel package not found in your wishlist");
    }

    for (auto i = wishlist_map.begin(); i != wishlist_map.end(); ) {
        if (i->second->get_id() == id) {
            i = wishlist_map.erase(i);
            break;
        }
        else {
            ++i;
